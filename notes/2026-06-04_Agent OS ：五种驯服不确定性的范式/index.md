...
| 3 | **信息分区** | Context Window 截断 = 永远只看到局部 | = 分区容忍，但分区是**永久的**而非暂时的 |
...
| 5 | 多 Agent 排序 | Event 时间戳 + trace\_id + Compaction | 约束空间 | O |
...
| Trace ID propagation | Agent trace\_id 贯穿 |
...
| A14 不确定度标注 | 信念置信度 <0.7 | 标注 `uncertain`, 优先 probing | 只读决策可容忍 0.5 |
