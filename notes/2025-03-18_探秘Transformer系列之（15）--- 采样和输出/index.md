...
另外，虽然从理论上讲，top\_p 似乎比 top\_k 更优雅，但这两种方法在实践中都很好用。top\_p 也可以与 top\_k 结合使用（当结合使用时，token 集首先被限制为 K 个候选，然后进一步缩小到满足概率累积阈值 *P* 的 token），这可以避免分数非常低的 Token，同时提供一些动态选择的空间。
...
| 参数 | 默认值 | 含义 |
| --- | --- | --- |
| top\_p | 0.95 | top-p概率阈值。如果top\_p小于1，则从高到低累加直到top\_p，取这前N个词作为候选 |
| top\_k | 50 | 保留前K个结果词作为候选 |
| repetition\_penalty | 1.0 | 重复处罚的参数。1.0意味着没有惩罚。 |
