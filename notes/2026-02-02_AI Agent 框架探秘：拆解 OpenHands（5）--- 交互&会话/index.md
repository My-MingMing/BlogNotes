...
![openhands-sdk](images/img_001.jpg)
...
1. **`Session`**：当前对话线程（可以将你与智能体的不同对话实例视为独立的**对话线程**，它们可能会利用**长期知识**）
   * 表示用户与你的智能体系统之间*单次、持续的交互*。
   * 包含该特定交互期间，智能体采取的消息和动作（称为 `Events`）的时间顺序序列。
   * 一个 `Session` 还可以保存仅在*本次对话*期间相关的临时数据（`State`）。
2. `State`：当前对话中的数据
   * 存储在特定 `Session` 内的数据。
   * 用于管理*仅*与*当前（单次）、活跃*对话线程相关的信息（例如，*本次对话*中的购物车商品，*本 Session* 中提到的用户偏好）。
   * 关注如何高效地读取、写入和管理 session 专属数据。
3. `Memory`：可检索的跨 Session 信息
   * 表示可能跨越*多个过去 Session*或包含外部数据源的信息存储。
...
一般来说，当用户开始与智能体交互时，`SessionService` 会创建一个 `Session` 对象。该对象作为*单个对话线程*相关所有内容的容器。其主要属性如下：
...
![Session lifecycle](https://adk.wiki/assets/session_lifecycle.png)
...
![openhands-4.1-1](images/img_003.png)
...
![openhands-4.1-2](images/img_004.png)
...
![openhands-4.1-3](images/img_005.png)
